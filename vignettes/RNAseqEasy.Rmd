...
```{r heatmap, echo=FALSE, fig.cap="Heatmap of normalized values of DEGs in the OPDA vs Mock comparison for Tak-1 genotype"}
...
-   A "topGO" folder, including results of GO enrichment analysis for Up, Down and Up+Down DEGs ($padj \le 0.05$, and $log2(FC) \ge 1$ for Up-regulated and $log2(FC) \le -1$ for Down-regulated genes):
    -   Two tab-delimited text files including results from the GO enrichement analysis. One include detailed results (topGO_Name_All/Up/Down_results.txt) and the other only enriched GO terms and their p-values (topGO_Name_All/Up/Down_pvals.txt).
...
```{r tree, echo=FALSE, fig.cap="Tree map of Gene Ontologies enriched in Up-regulated genes"}
ruta_pdf_generado <- file.path(output_Dir_DESeq2, "topGO", "topGO_Tak1_OPDA_Up_Treemap.pdf")
rutas_png_generadas <- pdftools::pdf_convert(
  pdf = ruta_pdf_generado,
  format = "png",
  dpi = 150 
...
```{r bubble, echo=FALSE, fig.cap="Bubble plot of Gene Ontologies enriched in Up-regulated genes"}
ruta_pdf_generado <- file.path(output_Dir_DESeq2, "topGO", "topGO_Tak1_OPDA_Up_bubble.pdf")
rutas_png_generadas <- pdftools::pdf_convert(
  pdf = ruta_pdf_generado,
  format = "png",
  dpi = 150 
