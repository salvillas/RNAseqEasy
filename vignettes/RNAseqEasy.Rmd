...
To perform GO semantic clustering, the analysis is based on rrvgo [@sayols2023] and GOSemSim [@yu2020] packages. The available species for this analysis are included in [Bioconductor webpage](https://bioconductor.org/packages/release/BiocViews.html#___OrgDb). The only plant organims included is *Arabidopsis thaliana*, so we will use its code (org.At.tair.db) for this analysis.
...
output_Dir_DESeq2 <- file.path(output_dir, "test", "DESeq2")
...
ruta_pdf_generado <- file.path(test_dir, "topGO", "topGO_test_DESeq2_Up_Scatterplot.pdf")
