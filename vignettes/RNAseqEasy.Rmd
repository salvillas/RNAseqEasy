...
```{r Diagram, fig.cap="RNAseqEasy workflow", echo = FALSE}
library(DiagrammeR)
...
```{r DESeq2_simple Diagram2, fig.cap="DESeq2_simple() function general scheme", echo = FALSE}
grViz("
digraph RNAseqEasy {
  graph [layout = dot, rankdir = TB]
...
```{r PCA, echo=FALSE, fig.cap="PCA plot of PC1 vs PC2, using 500 top genes", out.height= 0.6}
...
```{r WGCNA_Modules_First, error = TRUE}
library(WGCNA)
...
```{r topGO_All Diagram2, fig.cap="topGO_All() function general scheme, echo = FALSE}
...
```{r WGCNA_Module Diagram2, fig.cap="WGCNA_Module() function general scheme, out.height= 0.5, echo = FALSE}
